...
ggplot(aes(y=at_bats, x=runs), data=mlb11) + geom_point()
```
...
```
```{r}
ggplot(aes(y=runs, x=wins), data=mlb11) + geom_point() + geom_smooth(method = 'lm')
```
...
ggplot(aes(y=runs, x=new_obs), data=mlb11) + geom_point() + geom_smooth(method = 'lm')
