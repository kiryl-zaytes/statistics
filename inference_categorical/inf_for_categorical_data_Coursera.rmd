...
ath <- us12%>% filter(response == 'atheist') %>% count(response)
ath$n / length(us12$response) 
```
...
spain_df <- atheism %>% filter(nationality == 'United States') %>% group_by(year)

inference(y = response ,data = spain_df, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
