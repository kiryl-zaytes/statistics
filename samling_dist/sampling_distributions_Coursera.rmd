...
ames %>%
  summarise(mu = mean(area), pop_med = median(area), 
            ...
            pop_q2 = quantile(area, 0.50), # first quartile, 25th percentile
...
ggplot(aes(area), data=samp1) + geom_histogram(binwidth = 250)
...
sample_means50 %>% summarise(xbar = mean(x_bar), med = median(x_bar))
```
...
<div id="exercise">
**Exercise**: Take a random sample of size 50 from `price`. Using this sample, what is your best point estimate of the population mean? 185037.8
</div>
```{r price-sample}
# type your code for this Exercise here, and Run Document

ggplot(aes(price), data=ames) + geom_histogram(binwidth = 50000)
m <-ames %>% sample_n(size=50) %>% summarise(x_bar=mean(price))
m
```

<div id="exercise">
**Exercise**: Since you have access to the population, simulate the sampling distribution for $\bar{x}_{price}$ by taking 5000 samples from the population of size 50 and computing 5000 sample means.  Store these means in a vector called `sample_means50`. Plot the data, then describe the shape of this sampling distribution. Based on this sampling distribution, what would you guess the mean home price of the population to be? 180000
</div>
```{r price-sampling}
# type your code for this Exercise here, and Run Document
sample_means50 <- rep_sample_n(ames,size=50, reps=5000, replace=TRUE) %>%
  summarise(x_bar=mean(price))

ggplot(aes(x_bar), data=sample_means50) + geom_histogram(binwidth = 10000)
...
# type your code for this Exercise here, and Run Document
sample_means150 <- rep_sample_n(ames,size=150, reps=5000, replace=TRUE) %>%
  summarise(x_bar=mean(price), se=sd(price), med= median(price))

ggplot(aes(x_bar), data=sample_means150) + geom_histogram(binwidth = 10000)

mean(sample_means150$se)
```
